...
exports.main = async (event, context) => {
  ...
  if (event.api === 'getMessages') {
    return await db.collection('test-messages').where({
      public: true
    }).get();
  }
  ...
  if (event.api === 'publish') {
    let access_token_Res = await uniCloud.httpclient.request(
      `https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=${appid}&secret=${appSecret}`, {
        dataType: 'json'
      });
    let access_token = access_token_Res.data.access_token;
    let textRes = await uniCloud.httpclient.request(
    ...
    return await db.collection('test-messages').add({
      content: event.content,
      time: dateTime,
